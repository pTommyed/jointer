...
void sending_tacho_request(int vesc_id) {
  ...
  request_buf[0] = CANvescID[0]; // nyní posílá žádost pouze vesc1 pro ostatní vesc nahradit --- CANvescID[vesc_id]
  //Serial.println(request_buf[0]);
  CAN.sendMsgBuf(0x00000+can_id+CANvescID[0], 1, 3, request_buf); // nyní pouze pro vesc 1 pro ostatní vesc nahradit ----- CAN.sendMsgBuf(0x00000+can_id+CANvescID[vesc_id], 1, 3, request_buf);
}
...
void Sending_tacho_buf_apu(int buf_position) {
  int i = 7;
  buf_position = buf_position * 2;
  
  for (int j=0;j<2;j++){
    i=i-j;
    //Serial.print(buf[i]);
    tacho_buf[i-buf_position] =buf[i];
  }
  //Serial.println("");
}
