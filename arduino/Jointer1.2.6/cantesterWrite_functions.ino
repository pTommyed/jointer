    ...
    if (RPM[CANvescID[1]]< RPMStart) {
      ...
      for (int i=0;i<2;i++){
        if (RPM[CANvescID[i]]>0) {
           ...
           CAN.sendMsgBuf(0x200+CANvescID[i], 0, 4, currentREVERS);
        }else {
           CAN.sendMsgBuf(0x00000100+CANvescID[i], 1, 4, currentOFF);
           CAN.sendMsgBuf(0x100+CANvescID[i], 0, 4, currentOFF);
        }
     }
   }
}
...
void CAN_MSG_Recev() {
  ...
  if (CANmessageID == 2305){ // zkontrolovat správnost adresy
    VescID = 1;
    CAN.sendMsgBuf(0x181, 0, 8, buf); // přeposílání zpráv - 901 -- 181
  }
  if (CANmessageID == 2306){ // zkontrolovat správnost adresy
    VescID = 2;
    CAN.sendMsgBuf(0x182, 0, 8, buf); // přeposílání zpráv - 902 -- 182
  }
